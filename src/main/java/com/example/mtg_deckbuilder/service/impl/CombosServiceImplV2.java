...
@Service
public class CombosServiceImplV2 implements ComboService {
    ...
    @Autowired
    public CombosServiceImplV2(PersonalLibraryServiceImpl personalLibraryService ) {
    ...
    public CardCombos findCombos(CustomUserDetails userId, LibraryFilters libraryFilters) throws Exception {
        ...
        Combos searchedCombos;
        if (libraryFilters.getCardName() == null || libraryFilters.getCardName().isBlank()) {
            searchedCombos = searchCombos(cards);
        }
        else {
            searchedCombos = searchCombos(libraryFilters.getCardName());
        }
    ...
    private CardCombos buildAllAvailableCombos(Combos searchedCombos) {
        ...
        if (searchedCombos.getResults().getIncluded() != null) {
            allVariants.addAll(searchedCombos.getResults().getIncluded());
        }
        if (searchedCombos.getResults().getAlmostIncluded() != null) {
            allVariants.addAll(searchedCombos.getResults().getAlmostIncluded());
        }
        if (searchedCombos.getResults().getAlmostIncludedByAddingColors() != null) {
            allVariants.addAll(searchedCombos.getResults().getAlmostIncludedByAddingColors());
        }

        List<ComboVariant> filteredVariants = allVariants.stream()
